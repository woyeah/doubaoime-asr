...
构建只在开发机，NAS 只拉镜像 + 启动，**不在 NAS 上做 git pull**。

| 文件 | 角色 |
|------|------|
| `docker-compose.yml` | 本地开发 |
| `docker-compose.prod.yml` | NAS 生产，`command: ["sleep", "infinity"]`——把它当库容器，exec 进去跑 |
| `Dockerfile` | python:3.11-slim + libopus0 + ffmpeg + `pip install -e .` |
| `.env`（gitignored） | `REGISTRY` / `NAS_USER` / `NAS_HOST` / `NAS_DIR`，所有环境字符串只在 .env，**脚本不带默认值** |
| `.env.example` | 模板 |
| `.deploy-state` | 缓存上次 build 的 commit hash，对比 `doubaoime_asr/ Dockerfile pyproject.toml` 决定是否跳 build |
| `deploy-local.sh` | 编排：判断是否需要 build → push → scp prod 编排 → ssh NAS `sudo sh -c '...PATH... docker compose pull && up'` |

NAS 上的 `.env` 由脚本写入，内容是 `REGISTRY` + `IMAGE_TAG`，供 `docker-compose.prod.yml` 插值出完整镜像名。

**Synology 两个非显然的坑（已处理在 deploy-local.sh）**：
- sshd 非交互 shell 不加载 `/etc/profile`，docker 不在 PATH → ssh 命令里显式 `export PATH=/usr/local/bin:/usr/bin:/bin:$PATH`
